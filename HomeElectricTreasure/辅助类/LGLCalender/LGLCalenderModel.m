//
//  LGLCalenderModel.m
//  LGLProgress
//
//  Created by 李国良 on 2016/10/11.
//  Copyright © 2016年 李国良. All rights reserved.
//  https://github.com/liguoliangiOS/LGLCalender.git

#import "LGLCalenderModel.h"
#import "LGLCalendarDate.h"
#import "LGLCalenderSubModel.h"
@implementation LGLCalenderModel

+ (void)getCalenderDataWithDate:(NSDate *)date block:(CallBackBlock)block {
    NSMutableArray * array = [NSMutableArray array];
    NSDate * cdate = date;
    NSInteger firstday = [LGLCalendarDate firstWeekdayInThisMonth:cdate]; // 本月的第一天是周几
    NSInteger totalDay = [LGLCalendarDate totaldaysInMonth:cdate];  //月份的所有天数
    NSInteger year = [LGLCalendarDate year:cdate];
    NSInteger month = [LGLCalendarDate month:cdate];
    
    for (NSInteger y = year; y < year + 2; y ++) {
        
        for (NSInteger i = month; i < 13; i++) {
            LGLCalenderModel * model = [[LGLCalenderModel alloc] init];
            model.year = y;
            model.month = i;
            model.firstday = firstday;
            for (NSInteger j = 1; j < totalDay  +1; j ++) {
                LGLCalenderSubModel * subModel = [[LGLCalenderSubModel alloc] init];
                subModel.day = j;
                subModel.price = [NSString stringWithFormat:@"%ld", 100 + j];
                [model.details addObject:subModel];
            }
            totalDay = [LGLCalendarDate totaldaysInMonth:[LGLCalendarDate nextMonth:cdate]];
            cdate = [LGLCalendarDate nextMonth:cdate];
            firstday = [LGLCalendarDate firstWeekdayInThisMonth:cdate];

            [array addObject:model];
        }
        month = 1;
    }
    
    block(array);
}

+ (void)getCalenderDataWithYear:(NSInteger)nowYear startMonth:(NSInteger)startMonth endMonth:(NSInteger)endMonth ElectricityCardHistoryListDataModel:(NSMutableArray<ElectricityCardHistoryListForm*>*)models block:(CallBackBlock)block{
    NSMutableArray * array = [NSMutableArray array];
    
    NSDateComponents *comp = [[NSDateComponents alloc]init];
    [comp setMonth:startMonth];
    [comp setDay:01];
    [comp setYear:nowYear];
    NSCalendar *myCal = [[NSCalendar alloc]initWithCalendarIdentifier:NSGregorianCalendar];
    NSDate *myDate = [myCal dateFromComponents:comp];
    
    NSDate * cdate = myDate;
    NSInteger firstday = [LGLCalendarDate firstWeekdayInThisMonth:cdate]; // 本月的第一天是周几
    NSInteger totalDay = [LGLCalendarDate totaldaysInMonth:cdate];  //月份的所有天数
    NSInteger year = nowYear;
    NSInteger month = [LGLCalendarDate month:cdate];
    
    
        for (NSInteger i = month; i < 13; i++) {
            if (i<startMonth || i>endMonth) {
                continue;
            }
            if (i-month == models.count) {
                break;
            }
            ElectricityCardHistoryListForm * form = models[i-month];
//            if (form.month != i) {
//                continue;
//            }
            LGLCalenderModel * model = [[LGLCalenderModel alloc] init];
            model.year = year;
            model.month = form.month;
            
            model.totalElecNumber = form.totalElecNumber;
            model.totalFee = form.totalFee;
            model.firstday = firstday;
            for (NSInteger j = 1; j < totalDay  +1; j ++) {
                LGLCalenderSubModel * subModel = [[LGLCalenderSubModel alloc] init];
                subModel.day = j;
                subModel.elecNum = @"";
                subModel.price = @"";
                if (j < [form.list.firstObject.day integerValue] || j > [form.list.lastObject.day integerValue]) {
                    [model.details addObject:subModel];
                    continue;
                }else if ([form.list[j-[form.list.firstObject.day integerValue]].day integerValue] != j) {
                    [model.details addObject:subModel];
                    continue;
                }
                subModel.elecNum=form.list[j-[form.list.firstObject.day integerValue]].elecNumber;
                subModel.price = form.list[j-[form.list.firstObject.day integerValue]].fee;
                [model.details addObject:subModel];
            }
            [array addObject:model];

            if (i-month< models.count-1) {
                ElectricityCardHistoryListForm * form2 = models[i-month+1];
                NSDateComponents *comp = [[NSDateComponents alloc]init];
                [comp setMonth:form2.month];
                [comp setDay:01];
                [comp setYear:nowYear];
                NSCalendar *myCal = [[NSCalendar alloc]initWithCalendarIdentifier:NSGregorianCalendar];
                NSDate *myDate = [myCal dateFromComponents:comp];
                
                NSDate * cdate = myDate;
                firstday = [LGLCalendarDate firstWeekdayInThisMonth:cdate]; // 本月的第一天是周几
                totalDay = [LGLCalendarDate totaldaysInMonth:cdate];  //月份的所有天数
                
                
//                totalDay = [LGLCalendarDate totaldaysInMonth:cdate];
////                cdate = [LGLCalendarDate nextMonth:cdate];
//                firstday = [LGLCalendarDate firstWeekdayInThisMonth:cdate];

            }
            
        }
    
    block(array);
}

- (NSMutableArray *)details {
    if (!_details) {
        _details = [NSMutableArray array];
    }
    return _details;
}
@end
