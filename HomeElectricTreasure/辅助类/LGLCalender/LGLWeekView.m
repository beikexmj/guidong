...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, frame.size.height)];
        imageView.image = [UIImage imageNamed:@"home_list_bac1"];
        [titleView addSubview:imageView];
//        titleView.backgroundColor = LGLColor(65, 207, 79);//LGLColor(19, 115, 113);
        for (NSInteger i = 0; i < 7; i++) {
            UIButton * btn = [UIButton buttonWithType:UIButtonTypeCustom];
            btn.frame = CGRectMake((WIDTH / 7) * i, 5, WIDTH / 7, frame.size.height - 10);
