...
-(void)setUserInfo:(NSDictionary *)userInfo{
    _userInfo = userInfo;
    UIImage *appIcon;
    appIcon = [UIImage imageNamed:@"登录logo"];
    if (!appIcon) {
        appIcon = [UIImage imageNamed:@"登录logo"];
    }
    ...
    self.content_label.text = self.userInfo[@"content"];
...
-(void)tapGesture:(UITapGestureRecognizer*)tapGesture{
    ...
    if ([StorageUserInfromation storageUserInformation].token) {
        ...
        [delegate.window.currentViewController.navigationController pushViewController:page animated:YES];
    }else{
        LoginViewController *controller = [[LoginViewController alloc] initWithNibName:@"LoginViewController" bundle:nil];
        controller.comeFromFlag = 3;
