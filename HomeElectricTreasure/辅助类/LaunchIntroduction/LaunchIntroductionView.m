...
-(void)createScrollView{
    ...
    for (int i = 0; i < images.count; i ++) {
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake((kScreen_width - (kScreen_width/375.0) *225)/2.0 , SCREEN_HEIGHT/2.0 - (kScreen_width/375.0) *225 - 10, (kScreen_width/375.0) *225, (kScreen_width/375.0) *225)];
        imageView.image = [UIImage imageNamed:images[i]];
        [superView addSubview:imageView];
    
        UIView *myView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT - 250, SCREEN_WIDTH, 150)];
        [superView addSubview:myView];
        UILabel * titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 21)];
        titleLabel.textColor =RGBCOLOR(48, 48, 48);
        titleLabel.font = [UIFont systemFontOfSize:25];
        titleLabel.textAlignment = NSTextAlignmentCenter;
        [myView addSubview:titleLabel];
        
        UILabel * subTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 50, SCREEN_WIDTH, 21)];
        subTitleLabel.textColor =RGBCOLOR(96, 96, 96);
        ...
        if (i == 0) {
            titleLabel.text = @"本地新闻资讯";
            subTitleLabel.text = @"最新资讯  电力通知  用电帮助";
        }else if (i == 1){
            titleLabel.text = @"便捷用电缴费";
            subTitleLabel.text = @"方便快捷的安全缴费渠道";
        }else{
            titleLabel.text = @"直观的用电查询";
            subTitleLabel.text = @"用电走势数据推送";
        }
    
    }
    UIButton *enterButton = [[UIButton alloc] initWithFrame:CGRectMake((SCREEN_WIDTH -150)/2.0, SCREEN_HEIGHT - 250 + 110, 150, 35)];
    [enterButton setTitle:@"立即体验" forState:UIControlStateNormal];
    [enterButton setTitleColor:RGBCOLOR(0, 167, 255) forState:UIControlStateNormal];
    enterButton.layer.cornerRadius = 5;
    enterButton.layer.borderColor = RGBCOLOR(0, 167, 255).CGColor;
    ...
    page.currentPage = 0;
    self.currentColor = RGBCOLOR(0, 167, 255);
