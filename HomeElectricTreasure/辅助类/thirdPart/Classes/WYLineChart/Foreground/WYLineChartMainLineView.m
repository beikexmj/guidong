...
@interface WYLineChartMainLineView ()<CAAnimationDelegate>
...
//        [self.layer insertSublayer:gradientLayer atIndex:(unsigned int)];
...
//            [rigionPath addLineToPoint:CGPointMake(lastPoint.x, boundsHeight)];
    ...
    if (_junctionStyle != kWYLineChartJunctionShapeNone && _showJunctionShape) {
        ...
        [_parentView.points enumerateObjectsUsingBlock:^(WYLineChartPoint * point, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (!(idx == 0 || idx == _parentView.points.count-1)) {
                WYLineChartJunctionShape *shape = [[WYLineChartJunctionShape alloc] initWithStyle:kWYLineChartJunctionShapeHollowCircle
                                                                                             size:_junctionSize
                                                                                           origin:CGPointZero];
...
//                                          forKeyPath:@"original"];
                ...
                WYLineChartPoint *point = _parentView.points[idx];
                label.text = [NSString stringWithFormat:@"%0.1f度",point.value];
                label.center = CGPointMake(point.x, point.y-13);
                label.textAlignment = NSTextAlignmentCenter;
                label.font = [UIFont systemFontOfSize:11];
                label.textColor = _junctionColor;
//                [self addScaleSpringAnimationForView:label reverse:false
//                                               delay:delay
//                                          forKeyPath:@"original"];
...
- (void)addScaleSpringAnimationForView:(UIView *)view reverse:(BOOL)isReverse delay:(CGFloat)delay forKeyPath:(NSString *)keyPath {
    ...
    animation.values = !isReverse ? @[@0.01, @1.5, @1.0] : @[@2.5, @0.01];
