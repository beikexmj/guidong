...
#define DEFAULT_JUNCTION_POINT_STYLE kWYLineChartJunctionShapeHollowCircle
...
- (void)initializeProperty {
    ...
    //                Color                  //
    //* * * * * * * * * * * * * * * * * * * *//
    self.backgroundColor = [UIColor whiteColor];//DEFAULT_BACKGROUND_COLOR;
...
//    [self addSubview:_yAxisView];
    ...
    x =0; //CGRectGetMaxX(_yAxisView.frame);
    ...
    UIView *xAxisView = [[UIView alloc]initWithFrame:frame];
    xAxisView.backgroundColor = [UIColor colorWithRed:2/255.0 green:187/255.0 blue:255/255.0 alpha:1];
    [self addSubview:xAxisView];

    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, height - self.calculator.xAxisLabelHeight+5, 60, self.calculator.xAxisLabelHeight)];
    ...
    frame = CGRectMake(0, 0, self.bounds.size.width, _calculator.drawableAreaHeight);
...
//    NSLog(@" begin %@", [formatter stringFromDate:[NSDate dateWithTimeIntervalSinceReferenceDate:[NSDate timeIntervalSinceReferenceDate]]]);
    ...
    frame = CGRectMake(0, _calculator.drawableAreaHeight-8, _contentScrollView.contentSize.width, 8);
