...
@interface JGPayPassWordView2 ()<UITextFieldDelegate>
@property (strong,nonatomic) NSArray *textArr;

@end

@implementation JGPayPassWordView2
- (void)awakeFromNib{
    [super awakeFromNib];
...
//        self.subPayView.frame = CGRectMake(10, SCREEN_HEIGHT/2.0 - 110 - (SCREEN_WIDTH - 20)/12.0 , SCREEN_WIDTH - 20, 190 + (SCREEN_WIDTH - 20)/6.0);
        self.subPayView.frame = CGRectMake(10, SCREEN_HEIGHT - keyBoardBound.size.height - 10 - 140 - (SCREEN_WIDTH - 20)/6.0 , SCREEN_WIDTH - 20, 140 + (SCREEN_WIDTH - 20)/6.0);
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (self.actStr.length > 5) {
        ...
        if ([self.delegate respondsToSelector:@selector(textFieldHaveSixNumberForView2)]) {
            [self.delegate textFieldHaveSixNumberForView2];
        }
