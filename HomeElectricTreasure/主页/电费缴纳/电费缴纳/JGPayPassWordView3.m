...
@interface JGPayPassWordView3 ()<UITextFieldDelegate>
@property (strong,nonatomic) NSArray *textArr;

@end

@implementation JGPayPassWordView3
- (void)awakeFromNib{
    [super awakeFromNib];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (self.actStr.length > 5) {
        ...
        if ([self.delegate respondsToSelector:@selector(textFieldHaveSixNumberForView3)]) {
            [self.delegate textFieldHaveSixNumberForView3];
        }
