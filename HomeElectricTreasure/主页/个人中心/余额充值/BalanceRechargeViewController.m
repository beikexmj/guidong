...
-(void)setOtherFromNib{
    ...
    selectflag = 1;//微信1 支付宝0
    moneyFloat = -1;
    self.myScrollView.contentSize = CGSizeMake(SCREEN_WIDTH, 564);
...
- (IBAction)selectTypeBtnClick:(id)sender {
    ...
    if (button.tag == 10) {
        self.weichatSelectOrNotImage.image = [UIImage imageNamed:@"形状-120"];
        self.zhifubaoSelectOrNotImage.image = [UIImage imageNamed:@"形状-120-副本-3"];
        selectflag = 1;
    }else if(button.tag == 20) {
        self.weichatSelectOrNotImage.image = [UIImage imageNamed:@"形状-120-副本-3"];
        self.zhifubaoSelectOrNotImage.image = [UIImage imageNamed:@"形状-120"];
        selectflag = 0;
    }
}
...
- (IBAction)payMoneyRecordsBtnClick:(id)sender {
    RechargeRecordViewController *page = [[RechargeRecordViewController alloc]init];
