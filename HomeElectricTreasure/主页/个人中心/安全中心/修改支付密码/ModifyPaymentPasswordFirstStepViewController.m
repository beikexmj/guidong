...
- (IBAction)comfirmBtnClick:(id)sender {
    ...
    NSDictionary *dict = @{@"timestamp":[StorageUserInfromation dateTimeInterval],@"password":self.password.text,@"passwordType":@"1",@"username":storage.username,@"smscode":self.identifyingCode.text,@"device":@"1"};
    [ZTHttpTool postWithUrl:@"resetPassword" param:dict success:^(id responseObj) {
        NSLog(@"%@",[responseObj mj_JSONObject]);
        NSString * str = [JGEncrypt encryptWithContent:[responseObj mj_JSONObject][@"data"] type:kCCDecrypt key:KEY];
        NSLog(@"%@",str);
        NSLog(@"%@",[DictToJson dictionaryWithJsonString:str]);
